...
export function SocialProofTicker({ reviews, services }: SocialProofTickerProps) {
  ...
  const handleOpenThumbLightbox = (review: Review, variant: "before" | "after") => {
    ...
    if (review.beforeSrc) {
      items.push({
        ...
        title: `${review.vehicle} · Before Treatment`,
        subtitle: `Swirl marks and road film on ${review.vehicle} (${review.author}, ${review.city})`,
        badge: "Before",
      });
    }
    if (review.afterSrc) {
      items.push({
        ...
        title: `${review.vehicle} · Flawless Ceramic Finish`,
        subtitle: `Mirror gloss and ceramic hydrophobic protection (${review.author}, ${review.city})`,
        badge: "After",
      });
    }
  ...
  return (
    <section id="reviews" aria-labelledby="reviews-title" className="mt-10">
      <SectionHeading
        id="reviews-title"
        eyebrow="Google reviews"
        title="Charlotte drivers, verified"
        trailing={
      ...
      <div className={cn(surface, "flex items-center gap-4 p-5")}>
        <p className="text-5xl font-semibold tracking-tight text-zinc-100">{siteConfig.googleRating.toFixed(1)}</p>
        <div className="min-w-0">
          <Stars rating={siteConfig.googleRating} className="size-5" />
          <p className="mt-1 text-sm text-zinc-300">{siteConfig.googleReviewCount} reviews on Google</p>
          <p className="mt-0.5 inline-flex items-center gap-1 text-xs font-medium text-emerald-400">
            <BadgeCheck aria-hidden className="size-3.5" />
            Every review from a verified customer
          </p>
        </div>
      </div>

      <div className="no-scrollbar relative -mx-4 mt-3 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_6%,black_94%,transparent)] motion-reduce:snap-x motion-reduce:snap-mandatory motion-reduce:overflow-x-auto">
        <ul className="flex w-max animate-marquee px-4 hover:[animation-play-state:paused] active:[animation-play-state:paused] motion-reduce:animate-none">
          {loop.map((review, index) => {
            ...
            return (
              <li
                ...
                className={cn("w-[19rem] shrink-0 snap-start pr-3 sm:w-[21rem]", isDuplicate && "motion-reduce:hidden")}
              >
                <ReviewCard
                  review={review}
                  serviceName={serviceNames.get(review.serviceId)}
                  onSelect={() => handleOpenReview(review)}
...
function ReviewCard({ review, serviceName, onSelect, onThumbClick }: ReviewCardProps) {
    ...
    >
      <header className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-zinc-100">{review.author}</p>
      ...
      <blockquote className="text-sm leading-relaxed text-zinc-300">&ldquo;{review.quote}&rdquo;</blockquote>
      ...
      <footer className="mt-auto flex items-center justify-between gap-3 pt-2">
        ...
        <div className="min-w-0 flex-1 text-right text-xs">
          <p className="truncate font-medium text-zinc-200">{review.vehicle}</p>
      ...
      <div className="mt-1 flex items-center justify-between border-t border-zinc-800/60 pt-2 text-[11px] text-zinc-500 group-hover:text-zinc-300 transition">
        <span>Click for full review</span>
